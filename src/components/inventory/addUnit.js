...
const AddUnit = (props) => {
    ...
    useEffect(() => {
        ...
        colorList.forEach(element => {
            temp.push(<div title={`${element.name} : ${element.hex}`} style={{ "backgroundColor": element.hex, "height": "30px", "width": "30px", "display": "inline-block", "padding-right": "10px" }} onClick={() => { setColor({ name: element.name, hex: element.hex }) }} />)});
        setColorPanel(temp)
    }, [])

    const submit = (e) => {
        var url = user.baseurl + '/vendor/inventory/unit/add';
        ...
        axios.post(url, {
            ...
            .then((response) => {
                ...
                if (response.status == 200) {
                    alert("Unit Successfully added successfully created.");
                    close();
                } else {
                    throw "Error";
                }
    ...
    const colorModal = () => {
        return (
            <div className="modal-container">
                <ButtonToolbar>
                    <Button onClick={() => open1()} appearance="primary">Select Color</Button>
                </ButtonToolbar>

                <Modal show={showColor} onHide={() => close1()} >
                    <Modal.Header>
                        <Modal.Title>Select Unit Color</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                ...
                <Container>
                    ...
                    <Form onSubmit={() => submit()} fluid>
                        <FormGroup>
                            ...
                            <div style={{ "backgroundColor": color.hex, "height": "20px" }} className="">
                            </div>
                            <h3>{color.name}</h3>
                            {colorModal()}
                        </FormGroup>
                        <FormGroup>
                            ...
                            <FormControl name="amount" required={true} value={count} onChange={(change) => setCount(change)} type="number" />
                            <HelpBlock tooltip>Number of units to add</HelpBlock>
                            <HelpBlock>Required</HelpBlock>
                        </FormGroup>
                        <FormGroup>
                            <ControlLabel>Size</ControlLabel>
                            <FormControl name="size" required={true} value={size} onChange={(change) => setSize(change)} type="number" />
                            <HelpBlock tooltip>Size of shoes, Gb shoe sizez</HelpBlock>
                            <HelpBlock>Required</HelpBlock>
                        </FormGroup>
                        <FormGroup>
                            <ButtonToolbar>
                                <Button type="submit" appearance="primary">Add Unit</Button>
                            </ButtonToolbar>
                        </FormGroup>
                    </Form>
                </Container>
    ...
    return (
        <div className="modal-container">
            <ButtonToolbar>
                <Button onClick={() => open()} appearance="primary">Add Unit</Button>
            </ButtonToolbar>

            <Modal show={show} onHide={() => close()} >
                <Modal.Header>
                    <Modal.Title>New Item Unit</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    {form()}
                </Modal.Body>
